let domUpdates = {
  welcomeUser(user, pantry, ingredientsData) {
    ...
    // let welcomeMsg = `
		...
		// </div>`;
    document.querySelector(".welcome-msg").innerText = `Welcome ${user.name.split(" ")[0]}!`;
  ...
  displayRecipeCards(currentRecipe, shortRecipeName) {
    ...
    <div class="recipe-card" id=${currentRecipe.id}>
      ...
      <div class="button-holder">
		...
		recipeCards.insertAdjacentHTML("beforeend", cardHtml);
  },
  ...
  addToMyRecipes(recipeData, user, fullRecipeInfo, allRecipes, ingredientsData, pantry) {
    ...
    else if (event.target.id === "instructions") { domUpdates.openRecipeInfo(event, fullRecipeInfo, allRecipes, ingredientsData, pantry) }
  },
  ...
  showToCookItems(recipeData, user) {
    this.showAllRecipes();
    let unsavedRecipes = recipeData.filter(recipe => {
